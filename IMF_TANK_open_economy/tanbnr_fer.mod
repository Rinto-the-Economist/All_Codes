...
% Author: ---
% Date: 25/12/2023
% Description: Two agent, nominal bond, nominal rigidities, fixed exchange rate
%
%----------------------------------------------------------------
...
% firm's profit
p_t * l^(1 - alpha) - w * l - phi / 2 * (p_t / p_t(-1) - 1)^2 * p_t * y_t - pi;   
...
c_art = s_t^eta * c_aro / (p_t^eta * (1 - s_t)^eta); % from tradeable-oil tradeoff
c_aht = s_t^eta * c_aho / (p_t^eta * (1 - s_t)^eta); 
