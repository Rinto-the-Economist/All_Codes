...
% Author: ---
% Date: 25/12/2023
% Description: Two agent, nominal bond, nominal rigidities, 
...
var c_aho c_aro c_bo c_aht c_art c_bt l_h l_r l y_t y_o p_t 
          e w pi n_a n_b r_star pi_r pi_h;
...
parameters alpha beta psi phi eta s_t theta rho gamma chi;
...
(1 - s_t) * c_aho^(-1 / eta) / e - s_t * c_aht^(-1 / eta) / p_t;

% consumption allocation between oil and tradeable for ricardian HHs in country a
(1 - s_t) * c_aro^(-1 / eta) / e - s_t * c_art^(-1 / eta) / p_t;

% consumption allocation between oil and tradeable in country b
% here, p_tstar = p_t / e by LOOP
(1 - s_t) * c_bo^(-1 / eta) - s_t * c_bt^(-1 / eta) / (p_t / e); 

% Euler equation for ricardian HHs in country a
c_aro^(-1 / eta) / ((s_t * c_art^(1 - 1 / eta) + 
     (1 - s_t) * c_aro^(1 - 1 / eta)) * e) - 
        beta * r_star *
            c_aro(+1)^(-1 / eta) / ((s_t * c_art(+1)^(1 - 1 / eta) + 
                 (1 - s_t) * c_aro(+1)^(1 - 1 / eta)) * e(+1));
...
psi - (1 - s_t) * c_aho^(-1 / eta) / ((s_t * c_aht^(1 - 1 / eta) + 
                 (1 - s_t) * c_aho^(1 - 1 / eta)) * e) * 
                           w * (1 - l_h);
                      
% labour supply decision for ricardian HHs in country a
psi - (1 - s_t) * c_aro^(-1 / eta) / ((s_t * c_art^(1 - 1 / eta) + 
                 (1 - s_t) * c_aro^(1 - 1 / eta)) * e) * 
                           w * (1 - l_r);

% htm's budget constraint in country a
p_t * c_aht + e * c_aho - w * l_h - pi_h;

% ricardian's budget constraint in country a
p_t * c_art + e * c_aro - w * l_r - pi_r - e * (n_a(-1) - n_a / r_star);

% HH's budget constraint in country b
c_bo + (p_t / e) * c_bt  - y_o - (n_b(-1) - n_b / r_star);

% firm's FOC
p_t * (1 - theta) * (1 - alpha) * l^((1 - theta) * (1 - alpha) - 1) * y_t^theta -
          w - phi * (p_t * y_t^theta * l(-1)^(theta * (1 - alpha)) / 
                (p_t(-1) * l^(theta * (1 - alpha)) * y_t(-1)^theta) - 1) *
                          p_t * y_t * (-theta) * (1 - alpha) * p_t * y_t^theta *
                                l(-1)^(theta * (1 - alpha)) / (p_t(-1) * 
                                      l^(theta * (1 - alpha) + 1) * y_t(-1)^theta); 
...
c_art = s_t^eta * e^eta * c_aro / (p_t^eta * (1 - s_t)^eta); % from tradeable-oil tradeoff
c_aht = s_t^eta * e^eta * c_aho / (p_t^eta * (1 - s_t)^eta); 
...
w = (1 - theta) * (1 - alpha) * p_t * 
          l^((1 - theta) * (1 - alpha) - 1) * y_t^theta; % from firm's FOC at SS
pi = p_t * l^(1 - alpha) - w * l; % from firm's profit at SS
pi_r = pi;
pi_h = (pi - chi * pi_r) / (1 - chi);
r_star = 1 / beta; % from SS Euler
n_a = r_star / ((r_star - 1) * e) * (p_t * c_art + e * c_aro - w * l - pi_r); % from the BC
...
stoch_simul(order=1, irf=100, solve_algo = 3, irf_plot_threshold=0) 
                      c_aho c_aro c_bo c_aht c_art c_bt l_h 
                             l_r l y_t y_o p_t e w pi n_a n_b r_star;
