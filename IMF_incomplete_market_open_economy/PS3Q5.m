...
%----------------------------------------------------------------
%
% Title: International Macro-Finance Problem Set 3, Q5 model file
% Author: ---
% Date: 10/12/2023
...
%----------------------------------------------------------------
%% 2. Set government debt as a parameter & foreign lending
%----------------------------------------------------------------
...
while err_tot > errtol_tot && iter_tot <= maxIter_tot
  ...
  policy.li = ((1 - guess.tau) .* w_p0 .* grid.s1).^(1 / par.eta);  
  l0 = pi * policy.li.';
  ...
  while err_r > errtol_r && iter_r <= maxIter_r
    ...
    c_constrained = (1 - guess.tau) .* w_p0 .* grid.s2 .* policy.li + ...
    ...
    while err_c > errtol_c && iter_c <= maxIter_c
      ...
      bi_state = (Ec ./ w_p0 + grid.b2 - (1 - guess.tau) .* grid.s2 .* ...
    ...
    bi_choice  = (grid.b2 * guess.r0 + (1 - guess.tau) * grid.s2 .* policy.li - ...
                guess.ci ./ w_p0); 
    ...
    % update guess for r
    b = sum(sum(policy.bi .* G0)); % aggregate HH lending
    c = sum(sum(policy.ci .* G0));
    b_star = -(b + par.bg); % solve for aggregate borrowing from bond market clearing
    tau_new = (par.g + par.bg * (1 - guess.r0)) / y; % solve for tax from govt BC
    guess.tau = lambda * tau_new + (1 - lambda) * guess.tau;

    r_new = (par.bg + par.g - guess.tau * y) / par.bg; % update r from govt BC
    % r_new = 1 + c / (b * w_p0) - (1 - guess.tau) * l0 / b; % solve for interest from HH BC
